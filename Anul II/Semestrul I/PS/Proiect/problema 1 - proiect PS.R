...
#1. Generati N = 1000 de realizări independente din fiecare repartitie
...
#Ilustrati grafic functiile de masă, respectiv functiile de densitate pentru fiecare din repartitiile din enuntul problemei. 
#Considerati câte 5 seturi de parametrii diferiti pentru fiecare repartitie si suprapuneti graficele pe aceeasi figură pentru fiecare repartitie.
...
legend("topright" , c("λ=0.5","λ=1","λ=8","λ=5", "λ=18"), col=c("green","goldenrod2","salmon","darkcyan","orchid"),pch=16)
...
legend("topright",c(expression("μ=30"~sigma~"=1"),expression("μ=5"~sigma~"=2"),expression("μ=5"~sigma~"=4"),expression("μ=45"~sigma~"=2"),expression("μ=23"~sigma~"=3")), col=c("green","goldenrod2","salmon","darkcyan","orchid"),pch=16)


#Reparitia Exponentiala
lambda1exp <- 0.25
lambda2exp <- 0.5
lambda3exp <- 1.2
lambda4exp <- 1.5
...
legend("topright" , c("λ=0.25","λ=0.5","λ=1.2","λ=1.5", "λ=1.8"), col=c("green","goldenrod2","salmon","darkcyan","orchid"),pch=16)



#3. Pentru seturile de parametri de la punctul anterior, trasati functiile de repartitie

#Functia repartitie pentru binomiala
...
#Functia repartitie pentru Poisson
...
legend(23,0.7,c("λ=0.5","λ=1","λ=8","λ=5", "λ=18"), col=c("green","goldenrod2","salmon","darkcyan","orchid"),pch=16)

#Functia repartitie pentru Noramala
...
legend("bottomright" , cex = 0.9 , c(expression("μ=30"~sigma~"=1"),expression("μ=5"~sigma~"=2"),expression("μ=5"~sigma~"=4"),expression("μ=45"~sigma~"=2"),expression("μ=23"~sigma~"=3")) , col = c("green" , "goldenrod2" , "salmon" , "darkcyan" , "orchid") , pch=16)

#Functia reparitie pentru Exponentiala
...
legend(6.4 , 0.75 , c("λ=0.25" , "λ=0.5" , "λ=1.2" , "λ=1.5" , "λ=1.8") , col = c("green" , "goldenrod2" , "salmon" , "darkcyan" , "orchid") , pch = 16)


#4. Aproximarea functiei de repartitie a binomialei

aproximareBinomiala <- function(n , p , k) # Functia care calculeaza Binomiala
{
  return (pbinom(k , n , p))
}
...
tabel <- function(n , p)
{
  ...
  for(i in 1:10)
  {
    binom[i] <- aproximareBinomiala(n , p , i)
...
#5. Pentru fiecare n ∈ {25, 50, 100} ilustrati pe acelasi grafic erorile maximale absolute dintre functia de repartitie binomială si cele patru aproximări, considerând 0.01 ≤ p ≤ 0.5.
...
kolmogorov <- function(n)
{
    ...
    for(p in seq(0.01 , 0.5 , 0.01))
    {
      ...
      for(k in 0:n)
      {
        bin <- c (bin , aproximareBinomiala (n , p , k))
...
#6.  Pentru 5 seturi de parametrii diferiti ilustrati grafic densitătile repartitiei normale-asimetrice, suprapunând graficele pe aceeasi figură.
...
{
  #dam valori lui μ, sigma si lambda
  u1 <- 25
  s1 <- 6
  lam1 <- 10
  u2 <- 1
  ...
  legend("topright" , c(expression("μ=25"~sigma~"=6"~lambda~"=10 ") ,expression("μ=1"~sigma~"=7"~lambda~"=53 "),expression("μ=32"~sigma~"=8 "~lambda~"=9 "),expression("μ=9"~sigma~"=19"~lambda~"=5 "),expression("μ=32"~sigma~"=14"~lambda~"=5 ")), col = c("green" , "goldenrod2" , "salmon" , "darkcyan" , "orchid") , pch=16)
}

graficNormalaAsimetrica()


#7 Creati o functie care să primească parametrii (n, p) ca valori de intrare si să întoarcă valorile parametrilor (µ, σ, λ)
...
#8. Pentru fiecare n ∈ {25, 50, 100} si fiecare p ∈ {0.05, 0.1} să se afiseze un tabel cu trei coloane (k,Binomiala, Normala Asimetrică) în care să apară aproximarea de mai sus pentru functia de repartitie si de masă a binomialei. 
#Evaluati grafic acuratetea aproximării conform punctului 5.
...
#Evaluati grafic acuratetea aproximării
