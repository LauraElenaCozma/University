...
def process_sample(line):
    ...
    string_without_numbers = re.sub(r'\d+', ' ', string_without_ne)
...
cv = TfidfVectorizer(preprocessor=process_sample , min_df=3, max_df=0.45, norm='l2' , lowercase = False)  # token_pattern = r'\b[^\d\W]+\b'
...
validation_target_samples_vect = []
for line in validation_target_samples:
    validation_target_samples_vect.append(process_sample(line))
print("vs ", len(validation_target_samples_vect))

validation_target_samples_vect = cv.transform(validation_target_samples_vect).toarray()
...
print("after fit")
predicted_labels_vt_svm = svm_model.predict(validation_target_samples_vect)
...
print('f1 score target ' , f1_score(np.asarray(validation_target_labels) , predicted_labels_vt_svm))
print('f1 score source ', f1_score(np.asarray(validation_source_labels), predicted_labels_vs_svm))
...
file = open('/content/drive/My Drive/data/resultcript.csv', 'w')  # creez fisier de tip csv
...
file.close()
files.download('/content/drive/My Drive/data/resultcript.csv')  # downloadez fisierul
