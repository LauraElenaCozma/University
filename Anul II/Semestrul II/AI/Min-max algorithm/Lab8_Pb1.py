...
class Joc:
    ...
    NR_LINII = 3
    NR_COLOANE = 3
    JMIN = None     #simbolurile pentru cei doi jucatori
    JMAX = None
    GOL = '#'       #simboluri pentru casuta necompletata

    def __init__(self, tabla=None):     #reprezentat ca lista simpla
    ...
    def final(self):
        ...
        rez = (elem_identice(self.matr[0:3])
               ...
               or elem_identice(self.matr[0:9:4])   #diagonala principala
               or elem_identice(self.matr[2:8:2]))  #diagonala secundara
        if (rez):
            return rez
        elif Joc.GOL not in self.matr:      #rez e False(remiza sau nu s-a terminat); daca jocul s-a terminat
            return 'remiza'
        else:
            return False
    ...
    def mutari_joc(self, jucator):
        ...
        ### DONE
        #linii
        for index in range(len(self.matr)):             #parcurgem tabla de joc
            if self.matr[index] == self.GOL:            #unde gasim primul loc liber
                config_noua = copy.deepcopy(self.matr)  #copiem tabla de joc in config_noua
                config_noua[index] = jucator            #adaugam simbolul corespunzator jocatorului curent pe pozitia libera gasita
                l_mutari.append(Joc(config_noua))       #adaugam noua configuratie in lista
